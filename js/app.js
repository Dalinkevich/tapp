class CollaborativeShoppingList {
  ...
  async loadBaseItems() {
    try {
      ...
      const saved = await this.cloudStorageGet(key);
      if (saved) {
  ...
  async saveBaseItems() {
    try {
      const key = `base_items_${this.listId}`;
      const data = JSON.stringify(this.baseItems);
      if (this.tg?.CloudStorage) {
        this.tg.CloudStorage.setItem(key, data);
      } else {
  ...
  async syncBaseToFirebase() {
    if (!this.listId) return;
    try {
      await fetch(`${this.firebaseUrl}/lists/${this.listId}/baseItems.json`, {
        method: 'PUT',
        body: JSON.stringify(this.baseItems)
      });
    } catch (e) {
      console.log('Синхронизация базы:', e);
    }
  }
    ...
    if (this.baseItems.length === 0) {
      container.innerHTML = `
        <div class="base-empty">
          <div class="base-empty-icon">📭</div>
          <p>Добавьте товары, которые покупаете регулярно</p>
        </div>
      `;
  ...
  toggleBaseItem(index) {
    if (this.selectedBaseItems.has(index)) {
  ...
  deleteBaseItem(index, event) {
    ...
    setTimeout(() => {
      ...
      this.selectedBaseItems.forEach(i => {
        if (i < index) newSelected.add(i);
        else if (i > index) newSelected.add(i - 1);
      });
      this.selectedBaseItems = newSelected;
      
      this.saveBaseItems();
      this.renderBaseList();
      this.haptic('rigid');
      this.showNotification('🗑️ Удалено из базового');
    }, 250);
  }

  async addSelectedBaseToCurrent() {
    if (this.selectedBaseItems.size === 0) {
    ...
    for (let index of this.selectedBaseItems) {
      ...
      if (text && !this.items.some(i => i.text.toLowerCase() === text.toLowerCase())) {
        this.items.unshift({
    ...
    if (added.length > 0) {
      ...
      this.haptic('success');
      this.showNotification(`✨ Добавлено: ${added.length}`);
      this.selectedBaseItems.clear();
  ...
  updateBaseCounter() {
    ...
    if (counter) {
      counter.textContent = `(${this.selectedBaseItems.size})`;
    }
  }
  ...
  startRealtimeSync() {
    setInterval(async () => {
      ...
      try {
        ...
        if (data.baseItems) {
          const newBaseJson = JSON.stringify(data.baseItems);
          const currentBaseJson = JSON.stringify(this.baseItems);
          if (newBaseJson !== currentBaseJson) {
            this.baseItems = data.baseItems;
            this.renderBaseList();
          }
        }
    ...
    if (baseModal) {
      ...
      if (baseInput) {
        baseInput.addEventListener('keypress', (e) => {
          if (e.key === 'Enter') {
            e.preventDefault();
            this.addToBase(baseInput.value);
          }
        });
      }
      
      document.getElementById('base-add-btn')?.addEventListener('click', () => {
        const input = document.getElementById('base-input');
        if (input) this.addToBase(input.value);
      });
      
      document.getElementById('base-add-to-current')?.addEventListener('click', () => {
        this.addSelectedBaseToCurrent();
      });
    }
  }
